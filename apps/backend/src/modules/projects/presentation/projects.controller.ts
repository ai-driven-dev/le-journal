...
import { USER_REPOSITORY, UserRepository } from '../../users/domain/user.repository.interface';
...
export class ProjectsController implements OnModuleInit {
  ...
  async createProject(@Body() createProjectDto: CreateProjectDto): Promise<ProjectDomain> {
    const project = await this.createProjectUseCase.execute(createProjectDto);

    const instance = plainToInstance(ProjectDomain, project);
